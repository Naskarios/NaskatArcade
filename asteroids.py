# https://gist.github.com/vinothpandian/4358972
# Enterprise pygame
...
class Ship:
    ...
    def shoot(self):
        ...
        missile_pos = [
            self.pos[0] + 40 + self.radius * self.forward[0],
            self.pos[1] + 40 + self.radius * self.forward[1],
        ]

        missile_vel = [
            self.vel[0] + 6 * self.forward[0],
            self.vel[1] + 6 * self.forward[1],
        ]
        missile_group.add(
            Sprite(
                missile_pos,
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            ...
            canvas.blit(
                rot_center(self.image, self.angle),
...
Enterprise = Ship(
    [WIDTH // 2, HEIGHT // 2], [0, 0], 0, ship_image, thrusted_ship_image, ship_info
)
rock_group = set([])
missile_group = set([])
explosion_group = set([])
    ...
    for element in list(group):
        if element.collide(other_object):
            explosion_group.add(
                Sprite(
                    [
                        element.get_position()[0] - 3 * element.radius,
                        element.get_position()[1] - 3 * element.radius,
                    ],
                    [0, 0],
                    0,
                    0,
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        while 1:
            ...
            if (
                dist(random_pos, Enterprise.get_position())
                <= rock_info.get_radius() + 2 * Enterprise.get_radius()
            ):
                continue
            else:
                rock_pos = random_pos
...
def click(event):
    ...
    if (
        event.button == 1
        and event.pos[0] in range(WIDTH // 2 - 200, WIDTH // 2 + 200)
        and event.pos[1] in range(HEIGHT // 2 - 150, HEIGHT // 2 + 150)
        and not started
    ):
        started = True
        lives = 3
